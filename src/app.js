...
function removeTodo(id) {
  saveListToLocalStorage(
    getPersistedListFromLocalStorage().filter((todo) => todo.id !== id)
  );
...
function toggleTodo(id) {
  saveListToLocalStorage(
    getPersistedListFromLocalStorage().map((item) =>
...
function editTodo(id) {
  ...
  textSpan.addEventListener('blur', function () {
    ...
    if (newText.trim().length) {
      const editedTodo = getPersistedListFromLocalStorage().map((item) =>
